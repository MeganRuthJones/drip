...
class GF_EmailOctopus extends GFFeedAddOn {
	 ...
	 *
	 * @since  1.0.0
	 * @since  1.4.0 Updated return value for consistency with other add-ons, so the framework can save the feed status to the entry meta.
	 *
	 * @access public
	 *
	...
	public function process_feed( $feed, $entry, $form ) {
		...
		if ( is_wp_error( $response ) ) {
			...
			if ( $this->is_double_opt_in( $list_id ) ) {
				$this->add_note( $entry['id'], sprintf( esc_html__( 'The user email has been sent a notification to subscribe. Contact ID: %s.', 'gravityformsemailoctopus' ), rgar( $response, 'id' ) ), 'success' );
			} else {
				$this->add_note( $entry['id'], sprintf( esc_html__( 'The user email has been added to your EmailOctopus list. Contact ID: %s.', 'gravityformsemailoctopus' ), rgar( $response, 'id' ) ), 'success' );
			}
		}
